...
The document payload preserves catalog fields such as `shopUrl`, `timing`, `storage`, `protocolRole`, `purchase`, and `retirement` unchanged. `shopUrl` is limited to official provider/shop pages. `timing` is source-backed informational guidance for morning/daytime/evening UI hints; it is not medical advice or a personalized schedule. `storage` distinguishes true refrigerator storage from ordinary cool/dry or room-temperature storage so the UI can show the subtle snowflake filter without implying that all cool-storage products belong in the fridge. `protocolRole` distinguishes a personal default, rotation item, cataloged alternative, or retired product. `purchase` records stable order provenance such as SKU and canonical order URL without treating a historical price as a current shop price. `retirement` links an inactive product to selected replacements and explicitly records any ingredients that are not directly replaced.
